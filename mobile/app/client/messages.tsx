...
export default function ClientMessagesScreen() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get(`/conversations/${user?.id}`, { params: { since: lastIdRef.current } }).then(r => {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#F6F3EE' }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
      <View style={s.header}>
