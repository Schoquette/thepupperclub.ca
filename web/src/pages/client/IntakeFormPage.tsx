            ...
            <div className="space-y-4">
              ...
              <FieldRow label="General Notes">
                {readOnly
                  ...
                  : <textarea className={`${fieldCls} resize-none`} rows={3} value={dog.general_notes} onChange={e => set({ general_notes: e.target.value })} placeholder="Anything else about your dog worth knowing &mdash; quirks, preferences, history…" />
                }
              </FieldRow>
            </div>
          </div>
...
export default function IntakeFormPage() {
  ...
  const { data: clientData, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/client/intake').then(r => r.data.data),
  });
  ...
  const isSubmitted = submitted || Boolean(clientData?.client_profile?.intake_submitted_at);
  const submittedAt: string | null = clientData?.client_profile?.intake_submitted_at ?? null;
  // Clients can keep editing their intake after submission — life
  // changes, dogs change, addresses change. The 'submitted' state is a
  // historical marker, not a lock. Saves go through as normal updates
  // without re-triggering the submit notification flow.
  ...
  const submitForm = useMutation({
    mutationFn: (f: FormData) => api.post('/client/intake/submit', f),
    onSuccess: () => {
  ...
  if (submitted && !submittedAt) {
    return (
      <div className="min-h-screen bg-cream flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-card p-10 max-w-md w-full text-center space-y-6">
          ...
          <p className="text-sm text-taupe">
            Your information has been submitted successfully. We will review it and get back to you shortly.
          </p>
    ...
    <div className="min-h-screen bg-cream">
      ...
      {isSubmitted && submittedAt && (
        <div className="bg-green-50 border-b border-green-200 px-6 py-2 text-sm text-green-700">
          Submitted on {new Date(submittedAt).toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' })}. You can update any field below at any time &mdash; we&rsquo;ll be notified of changes.
        </div>
      )}
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <SectionCard id="section-6">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <p className="text-xs text-taupe mb-3">How would you like to receive updates about visits, report cards, and other notifications?</p>
              {readOnly ? (
                <div className="flex flex-wrap gap-2">
        ...
        <SectionCard id="section-7">
          ...
          <p className="text-sm text-taupe mb-4 leading-relaxed">
            Anything else we should know? This is the catch-all for the
            things that don&rsquo;t fit anywhere else &mdash; household
            quirks, schedule notes, special requests, context that helps
            us care for your dogs well.
          </p>
        ...
        <div className="flex justify-end gap-3 pb-10">
          {isSubmitted ? (
            // Post-submission: single Save button. Saves go through as
            // normal updates without re-firing the submit notification.
            <Button
              loading={saveDraft.isPending}
              disabled={!isDirty}
              onClick={() => form && saveDraft.mutate(form)}
