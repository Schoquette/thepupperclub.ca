...
function VetSection({ form, update, fieldCls }: { form: any; update: (p: any) => void; fieldCls: string }) {
  const [vetParts, setVetParts] = useState(() => {
    const parts = (form.vet_address || '').split(',').map((s: string) => s.trim());
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <input type="tel" className={fieldCls} value={form.vet_phone} onChange={(e: any) => update({ vet_phone: e.target.value })} placeholder="(604) 555-0200" />
      </div>
      <AddressAutocomplete
        value={vetParts}
        onChange={syncToForm}
            ...
            <div className="space-y-4">
              ...
              <FieldRow label="Petsitting Notes">
                {readOnly
                  ...
                  : <textarea className={`${fieldCls} resize-none`} rows={3} value={dog.petsitting_notes} onChange={e => set({ petsitting_notes: e.target.value })} placeholder="Anything we should know for overnight or in-home sitting — routines, sleep arrangements, separation anxiety, comfort items…" />
                }
              </FieldRow>
...
export default function IntakeFormPage() {
  ...
  const saveDraft = useMutation({
    mutationFn: (f: FormData) => api.post(`/admin/clients/${id}/intake/save`, f),
    onSuccess: () => {
      setIsDirty(false);
      setSaveError('');
      setSavedFlash(true);
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-espresso">
            Are you sure you want to submit this intake form for <strong>{clientName}</strong>? The profile can still be updated after submission.
          </p>
          {submitForm.isError && (
